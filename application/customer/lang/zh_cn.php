<?php
/**
 * Created by PhpStorm.
 * User: guo
 * Date: 2018/3/13
 * Time: 17:57
 */
return [
    //user.php 验证文案翻译
    'ID illegal' => '非法ID',
    'username is required' => '用户名只允许字母+数字+"_"形式',
    'This username has been used' => '用户名已存在',
    'email is required' => '请填写您的邮箱地址',
    'email format Error' => '请填写正确的邮箱地址',
    'This e-mail has been used' => '此邮箱已注册',
    'first name is required' => '请填写您的姓名',
    'first name is max 20' => '姓名最长为20个字符',
    'gender is require' => '请选择您的性别',
    'gender is error' => '请选择您的性别',
    'birthday is required' => '请选择您的生日',
    'phone number is require' => '请填写您的手机号',
    'This phone number has been used' => '电话号码已被注册',
    'Mobile format Error' => '请填写正确的手机号',
    'password is to easy' => '密码长度为6-20个字符，且必须包含字母和数字',
    'Country is required' => '请选择您的国家',
    'Zip code is required' => '请填写邮政编码',
    'billing address too long' => '账单地址太长',
    'delivery address too long' => '收件地址太长',
    'Verification Code is require' => '请填写您的验证码',
    'Why do I need to fill in this blank?' => '为什么我需要填写此项？',

    'logout' => '退出登录',
    'This e-mail already existed' => '邮箱已存在',
    'This phone number already existed' => '手机号已存在',
    //注册登录用到的词汇
    'Sign up' => '注册',
    'Already have an account?' => '已有账号？',
    'Sign in' => '登录',
    'Experience Center' => '用户体验中心',
    'Service' => '服务支持',
    'Warranty Card' => '保修卡',
    'Driver and firmware' => '驱动/固件',
    'Ticket' => '工单',
    'USER INFO' => '用户信息',
    'Next' => '下一步',
    'Exit' => '退出',
    'Message' => '消息',
    'Tips' => '温馨提示:',
    'Welcome to wavlink' => '欢迎使用Wavlink 公司出品的电子产品',
    'notice for login' => '<p>欢迎登陆注册WAVLINK用户中心；注册登陆后您将享受以下服务</p><p>1、产品保修服务</p><p>2、优惠福利活动抢先看</p><p>3、新品上市通知dffd</p><p>4、半价测试</p><p>这段文案还需要市场部核对后发布</p>',
    'Email' => 'E-mail',
    'username'=>'用户名',
    'phone'=>'手机',
    'send'=>'发送',
    'Change Email address' => '修改邮箱',
    'login' => '登录',
    'Password' => '密码',
    'New password' => '新密码',
    'Change Password' => '修改密码',
    'Remember_Me' => '记住密码',
    'Register' => '注册',
    'No account yet' => '还没有账号？',
    'Retrieve your password' => '找回密码',
    'Reset your password' => '重置您的密码',
    'Please input the correct phone number or e-mail address' => '请输入正确的邮箱地址',
    'Please input the correct e-mail address' => '请输入正确的邮箱地址',
    'Please input the correct phone number' => '请输入正确的手机号码',
    'Or' => '或者',
    'Reset Password' => '重置密码',
    'Please enter your phone number or e-mail address' => '请输入您的邮箱',
    'Please enter your name' => '请输入用户名',
    'Please enter your password' => '请输入您的密码',
    'Please enter your password again' => '请再次输入您的密码',
    'Please enter your e-mail address' => '请输入您的邮箱',
    'Please enter your e-mail captcha' => '请输入您的验证码',
    'Please enter the correct captcha' => '请输入正确验证码',
    'Please click \"Send\" to receive the verification code.' => '请单击“发送”接收验证码',
    /*注册验证文案*/
    'user name is required'=>'姓名必须填写',
    'password is required'=>'密码必须填写',
    'Password must be at least 6 letters'=>'密码必须输入大于6个字符',
    'Password cannot be longer than 20 letters'=>'密码必须输入小于20个字符',
    'Password verification' => '密码长度为6-20个字符，且必须包含字母和数字',
    'Input password again'=>'再输入一边密码',
    'The password is different twice'=>'两次密码输入不一致',
    'Please enter a correct email'=>'请输入正确的邮箱格式',
    'Please enter the correct email captcha'=>'请输入正确的验证码',
    'I accept the'=>'我同意',
    'and'=>'和',
    'Privacy agreement'=>'隐私条款',
    'Subscribe me'=>'订阅以获取最新优惠推送及更多资讯',
    'Agree is required' => '请先阅读并勾选同意我们的服务条款和隐私政策',
    'robot code tips' => '完成人机验证后，我们会给您发送一封包含验证码的邮件，此验证码将用于我们WAVLINK用户中心的注册/找回密码。如果您未收到邮件，请仔细检查您填写的邮箱是否正确，部分地区由于网络波动，收到邮件可能会比较慢，请耐心等候。如果两分钟后您任未收到邮件，请重新发送验证码试下，或者联系我们',
    /*注册验证文案结束*/
    'Input your first name'=>'输入姓名',
    'Input your E-mail' => '请输入您的E-mail',
    'Input your phone number' => '输入手机号',
    'Forgot Your Password?' => '找回密码',
    'Input your password' => '请输入您的密码',
    'Input your password again' => '再次确认您的密码',
    'confirm' => '确认密码',
    'captcha' => '验证码',
    'The verification code is invalid' => '请输入验证码',
    'Captcha Error' => '验证码错误',
    'The captcha is invalid' => '验证码错误',
    'Please input the captcha in the image on the right' => '请输入右边的图像所示验证码！',
    'Click change code' => '点击更换验证码',
    'Disclaimer' => '免责申明',
    'Terms Of Use Notice' => '服务条款',
    'GO' => '点击提交',
    'next'=>'下一步',
    'Already existed' => '已存在或者没有输入',
    'Your Email address Already existed' => '您输入的邮箱地址已经被注册了',
    'Your phone  Already existed' => '该手机号已经被注册了，请返回登录窗口',
    'Ok' => '确定',
    'User does not exist' => '用户不存在',
    'This Email address is not registered'=>'您输入的邮箱还没有在我们的系统注册',
    'Password Error' => '密码错误！',
    'Old Password input Error' => '旧密码输入错误',
    'Success' => '成功',
    'Unknown Error' => '未知错误',
    'failed' => '',
    'Send Error' => '发送失败',
    'Base Info' => '基本信息',
    //info label
    'First name' => '姓',
    'Last name' => '名',
    'Title' => '称呼',
    'Country' => '国家',
    'Tel' => '联系电话',
    'ZipCode' => '邮政编码',
    'Professional' => '专业兴趣',
    'Billing address' => '账单地址',
    'Delivery address' => '收件地址',
    'Where to' => '从哪种途径知道我们的？',
    'Old Password' => '旧密码',
    'New Password' => '新密码',
    'Save' => '保存',
    //info 内容提示
    'Input your last name' => '请输入名字',
    'Input your telephone number' => '输入您的手机或者电话',
    'Input your Zip code' => '输入您的邮政编码',
    'Input your Billing address' => '请输入您的账单地址',
    'Input your Delivery address' => '请输入您的收件地址',
    'Input your new password' => '请输入您的新密码',
    'Where did you learn about us?' => '您从哪里了解到我们的呢？',
    'Please choose your country' => '请选择您所在的国家',
    'Base info tips' => '<p>1、修改邮箱后需要重新登录，密码还是原来您登录进来的密码；</p>',
    //登录页面用到的
    'home' => '首页',
    'product' => '产品',
    'driver' => '驱动',
    'Wait a moment' => '稍等片刻',

    /**
     * 产品注册翻译
     */
    'Date Purchased' => '购买日期',
    'Purchase Platform' => '购买平台',
    'Skip' => '跳过',
    'Add product cases'=>'添加产品信息',
    'Where to buy'=>'购买平台',
    'select'=>'请选择',
    //邮件--找回密码的邮件内容
    'Dear' => '亲！',
    'noreplay' => '这封邮件由wavlink系统发出，他不接受回复',
    'Wavlink Support' => 'Wavlink 网站技术团队',
    'If the above connection cannot be opened' => '<p>如果上述连接无法打开，请复制如下连接到浏览器中打开（推荐使用谷歌浏览器）</p>',
    'Your email address is ' => '您输入的邮箱地址是',
    'Send Mail is Success' => '邮件发送成功，请您去您的邮件箱查收并重置密码',
    'Please' => '请',
    'Click me' => '点击此处',
    'Reset' => '重设密码',
    //保修功能的翻译内容
    'Warranty' => '保修',
    'Register Product' => '注册产品',
    'Sn' => 'SN',
    'Purchase Time' => '购买时间',
    'Model' => '型号',
    'Category' => '分类',
    'Warranty Status' => '保修状态',
    'Warranty Action' => '保修操作',
    'Extend' => '延保',
    'Apply' => '申请保修',
    'The “Purchase Time” has to be later than the Date of Manufacture in your registration.' => '购买时间不能比出厂时间还早！',
    'Warranty tips' => '<h4>温馨提示</h4>
        <p>1、若您想在wavlink官网享受质保服务，请您先完成产品注册，注册成功后您将在wavlink官网享受到延保和保修服务;</p>
        <p>2、请您在产品注册时填写准确无误的产品相关信息，若因您填写的信息与实际不相符将导致您无法完成产品注册；</p>
        <p>3、在您申请产品保修时，请确保您的产品仍在保修期内，当您成功申请保修后，我们的客服人员将尽快以邮件的方式与您取得联系。</p>',
    'Product reg tips' => '<h4>提示：</h4>
                <ul>
                    <li>1、*为必填项</li>
                    <li>2、可以阅读这篇文章产看自己的产品型号<a href="https://www.wavlink.com/zh_cn/faq/details/how_to_check_the_product_model.html" target="_blank">如何查看睿因产品型号？</a> </li>
                </ul>',
    'SN is required' => 'SN 为必填项，且SN 长度须为十五位。',
    'SN is max 15 bit'=>'SN 长度须为十五位。',
    'Has been registered'=>'该产品已经被注册了',
    //产品注册相关的
    'Order Number' => '订单号',
    'Please enter your product model' => '请输入您的产品型号',
    'Please enter your sn code' => '请输入您的产品SN',
    'Where did you buy our products' => '购买渠道',
    'Please enter your order number' => '请输入您的订单编号',
    'Products under review cannot perform extended warranty operation.'=>'正在审核中的产品无法执行延保操作。',
    'If you are in the process of enjoying the warranty service, you can’t perform the extended warranty operation.'=>'若您正处于保修服务的使用过程中，将不能执行延保操作。',
    'The extended warranty operation can only be performed once. It is not allowed to prolong the extended warranty by repeating the operation.'=>'延保操作只能执行一次，不可通过重复操作来增加延保时长。',
    'The security code is invalid.'=>'验证码错误！',
    'Message Sent Successfully!'=>'消息发送成功',
    'Message Sent Failed!'=>'消息发送失败！',
    //sn验证块的翻译
    'The length of Serial Number is invalid.'=>'序列号长度错误',
    'Serial Number Error, the fourth o sixth number is incorrect.'=>'序列号错误，第四位到第六位的数据信息有误。',
    'Serial Number Error'=>'序列号错误',
    'Number Segment Error'=>'序列号号段错误',
    //信息页翻译
    'My Account' => '我的账户',
    'Personal Profile' => '个人信息',
    'Security' => '账户安全',
    'Member Center' => '会员中心',
    'My Product' => '我的产品',
    'Basic info, like your name and photo, that you use on WAVLINK services' => '您在WAVLINK服务上使用的基本信息，例如您的姓名和照片',
    'Basic Info' => '基本信息',
    'User Name'=>'用户名',
    'Real Name' => '姓名',
    'Gender' => '性别',
    'Modify' => '修改',
    'Birthday' => '生日',
    'Contact Info' => '联系信息',
    'ZIP Code' => '邮政编码',
    'Delivery Address' => '收件地址',
    'Modify Your Real Name' => '填写您的姓名',
    'First Name' => '姓名',
    'Modify Your Gender' => '选择您的性别',
    'Modify Your Birthday' => '选择您的生日',
    'Please note that your birthday can only be modified once.' => '生日仅能设置一次，请谨慎填写',
    'Do not select a date before 1940.' => '生日选择请选择1940年之后的日期',
    'Year' => '年',
    'Month' => '月',
    'Day' => '日',
    'Please select the full date' => '请填写完整的生日信息',
    'Modify Your Country' => '选择您的国家',
    'Modify Your Phone Number' => '填写您的手机号',
    'Please note that the phone number cannot be changed after it is confirmed' => '电话号码仅能设置一次，请谨慎填写',
    'Modify Your E-mail' => '填写您的邮件地址',
    'email format is Error' => '邮件地址格式错误',
    'Please note that you should use the new e-mail address to log in after it is changed' => 'E-mail地址默认为注册账户邮箱，修改E-mail地址同时会修改注册账户。邮箱仅能设置一次，请谨慎填写',
    'Modify Your Zip Code' => '填写您的邮政编码',
    'Modify Your Delivery Address' => '填写您的收件地址',
    'ReWrite' => '重填',
    'Submit' => '提交',
    'Cancel' => '关闭',
    'Female'=>'女',
    'Male'=>'男',
    'Secret'=>'保密',
    'Please keep your password. After changing the password, you need to log in again.'=>'密码修改后，您需要重新登录，请牢记您的密码',
    'Your verification code is' => '您的验证码是',

    //产品列表
    'Product List' => '产品列表',
    'Add Products' => '添加产品',
    'delete product confirm' => '您确定删除该产品吗？',
    'delete product' => '删除产品',

    //国家 名称翻译
    'china' => '中国',
    'Afghanistan' => '阿富汗',
    'Albania' => '阿尔巴尼亚',
    'Algeria' => '阿尔及利亚',
    'American Samoa' => '美属萨摩亚',
    'Andorra' => '安道尔',
    'Angola' => '安哥拉',
    'Anguilla' => '安圭拉',
    'Antarctica' => 'Antarctica（南极地区）',
    'Antigua and Barbuda' => '安提瓜和巴布达',
    'Argentina' => '阿根廷',
    'Armenia' => '亚美尼亚',
    'Aruba' => '阿鲁巴',
    'Australia' => '澳大利亚',
    'Austria' => '奥地利',
    'Azerbaijan' => '阿塞拜疆',
    'Bahamas' => '巴哈马群岛',
    'Bahrain' => '巴林',
    'Bangladesh' => '孟加拉国',
    'Barbados' => '巴巴多斯',
    'Belarus' => '白俄罗斯共和国',
    'Belgium' => '比利时',
    'Belize' => '伯利兹',
    'Benin' => '贝宁共和国',
    'Bermuda' => '百慕大群岛',
    'Bhutan' => '不丹',
    'Bolivia' => '玻利维亚',
    'Bosnia and Herzegovina' => '波斯尼亚和黑塞哥维那波斯尼亚',
    'Botswana' => '博茨瓦纳',
    'Bouvet Island' => '布韦岛',
    'Brazil' => '巴西',
    'British Indian Ocean Territory' => '英属印度洋领地',
    'Brunei Darussalam' => '文莱达鲁萨兰国',
    'Bulgaria' => '保加利亚',
    'Burkina Faso' => '布基纳法索',
    'Burundi' => '布隆迪共和国',
    'Cambodia' => '柬埔寨共和国',
    'Cameroon' => '喀麦隆',
    'Canada' => '加拿大',
    'Cape Verde' => '佛得角共和国',
    'Cayman Islands' => '开曼群岛',
    'Central African Republic' => '中非共和国',
    'Chad' => '乍得共和国',
    'Chile' => '智利共和国',
    'christmas island' => '圣诞岛',
    'Cocos (Keeling) Islands' => '科科斯群岛',
    'Colombia' => '哥伦比亚',
    'Comoros' => '科摩罗伊斯兰联邦共和国',
    'Congo' => '刚果共和国',
    'Cook Islands' => '库克群岛',
    'Costa Rica' => '哥斯达黎加',
    'Cote D\'Ivoire' => '科特迪瓦',
    'Croatia' => '克罗地亚',
    'Cuba' => '古巴共和国',
    'Cyprus' => '塞浦路斯共和国',
    'Czech Republic' => '捷克共和国',
    'Denmark' => '丹麦王国',
    'Djibouti' => '吉布提共和国',
    'Dominica' => '多米尼克国',
    'Dominican Republic' => '多米尼加共和国',
    'East Timor' => '东帝汶民主共和国',
    'Ecuador' => '厄瓜多尔',
    'Egypt' => '阿拉伯埃及共和国',
    'El Salvador' => '萨尔瓦多共和国',
    'Equatorial Guinea' => '赤道几内亚',
    'Eritrea' => '厄立特里亚',
    'Estonia' => '爱沙尼亚共和国',
    'Ethiopia' => '埃塞俄比亚',
    'Falkland Islands (Malvinas)' => '福克兰群岛（Malvinas）',
    'Faroe Islands' => '法罗群岛',
    'Fiji' => '斐济共和国',
    'Finland' => '芬兰',
    'France, Metropolitan' => 'France, Metropolitan',
    'French Guiana' => '法属圭亚那',
    'French Polynesia' => '法属波利尼西亚',
    'French Southern Territories' => '法属南部领土',
    'Gabon' => '加蓬共和国',
    'Gambia' => '冈比亚共和国',
    'Georgia' => '格鲁吉亚',
    'Germany' => '德国',
    'Ghana' => '加纳共和国',
    'Gibraltar' => '直布罗陀',
    'Greece' => '希腊',
    'Greenland' => '格林兰',
    'Grenada' => '格林纳达',
    'Guadeloupe' => '法属瓜德罗普',
    'Guam' => '关岛',
    'Guatemala' => '危地马拉共和国',
    'Guinea' => '几内亚共和国',
    'Guinea-Bissau' => '几内亚比绍共和国',
    'Guyana' => '圭亚那合作共和国',
    'Haiti' => '海地',
    'Honduras' => '洪都拉斯共和国',
    'Hong Kong' => '中国香港',
    'Hungary' => '匈牙利',
    'Iceland' => '冰岛共和国',
    'India' => '印度',
    'Indonesia' => '印度尼西亚共和国',
    'Iran (Islamic Republic of)' => '伊朗伊斯兰共和国',
    'Iraq' => '伊拉克共和国',
    'Ireland' => '爱尔兰共和国',
    'Israel' => '以色列',
    'Italy' => '意大利',
    'Jamaica' => '牙买加',
    'Japan' => '日本',
    'Jordan' => '约旦哈希姆王国',
    'Kazakhstan' => '哈萨克斯坦共和国',
    'Kenya' => '肯尼亚共和国',
    'Kiribati' => '基里巴斯共和国',
    'North Korea' => '朝鲜',
    'Korea, Republic of' => '韩国',
    'Kuwait' => '科威特',
    'Kyrgyzstan' => '吉尔吉斯共和国',
    'Lao People\'s Democratic Republic' => '老挝',
    'Latvia' => '拉脱维亚共和国',
    'Lebanon' => '黎巴嫩',
    'Lesotho' => '莱索托王国',
    'Liberia' => '利比里亚共和国',
    'Libyan Arab Jamahiriya' => '阿拉伯利比亚民众国',
    'Liechtenstein' => '列支敦士登公国',
    'Lithuania' => '立陶宛共和国',
    'Luxembourg' => '卢森堡大公国',
    'Macau' => '中国澳门',
    'FYROM' => '马其顿共和国',
    'Madagascar' => '马达加斯加',
    'Malawi' => '马拉维共和国',
    'Malaysia' => '马来西亚',
    'Maldives' => '马尔代夫全称',
    'Mali' => '马里共和国',
    'Malta' => '马耳他共和国',
    'Marshall Islands' => '马绍尔群岛',
    'Martinique' => '马提尼克（法属）',
    'Mauritania' => '毛里塔尼亚伊斯兰共和国',
    'Mauritius' => '毛里求斯共和国',
    'Mayotte' => '马约特',
    'Mexico' => '墨西哥',
    'Micronesia, Federated States of' => '密克罗尼西亚联邦',
    'Moldova, Republic of' => '摩尔多瓦共和国',
    'Monaco' => '摩纳哥',
    'Mongolia' => '蒙古',
    'Montserrat' => '蒙塞拉特',
    'Morocco' => '摩洛哥王国',
    'Mozambique' => '莫桑比克',
    'Myanmar' => '缅甸联邦共和国',
    'Namibia' => '纳米比亚共和国',
    'Nauru' => '瑙鲁共和国',
    'Nepal' => '尼泊尔联邦民主共和国',
    'Netherlands' => '尼德兰王国',
    'Netherlands Antilles' => '荷属安的列斯群岛',
    'New Caledonia' => '新喀里多尼亚',
    'New Zealand' => '新西兰',
    'Nicaragua' => '尼加拉瓜共和国',
    'Niger' => '尼日尔共和国',
    'Nigeria' => '尼日利亚联邦共和国',
    'Niue' => '纽埃岛',
    'Norfolk Island' => '诺福克岛',
    'Northern Mariana Islands' => '北马里亚纳群岛自由邦',
    'Norway' => '挪威王国',
    'Oman' => '阿曼苏丹国',
    'Pakistan' => '巴基斯坦伊斯兰共和国',
    'Palau' => '帕劳（帛琉）共和国',
    'Panama' => '巴拿马共和国',
    'Papua New Guinea' => '巴布亚新几内亚独立国',
    'Paraguay' => '巴拉圭共和国',
    'Peru' => '秘鲁共和国',
    'Philippines' => '菲律宾',
    'Pitcairn' => '皮特凯恩群岛',
    'Poland' => '波兰',
    'Portugal' => '葡萄牙共和国',
    'Puerto Rico' => '波多黎各自治邦',
    'Qatar' => '卡塔尔国',
    'Reunion' => '留尼汪',
    'Romania' => '罗马尼亚',
    'Russian Federation' => '俄罗斯',
    'Rwanda' => '卢旺达',
    'Saint Kitts and Nevis' => '圣基茨和尼维斯',
    'Saint Lucia' => '圣卢西亚',
    'Saint Vincent and the Grenadines' => '圣文森特和格林纳丁斯',
    'Samoa' => '萨摩亚群岛',
    'San Marino' => '圣马力诺共和国',
    'Sao Tome and Principe' => '圣多美和普林西比',
    'Saudi Arabia' => '沙特阿拉伯',
    'Senegal' => '塞内加尔',
    'Seychelles' => '塞舌尔共和国',
    'Sierra Leone' => '塞拉利昂共和国',
    'Singapore' => '新加坡',
    'Slovak Republic' => '斯洛伐克共和国',
    'Slovenia' => '斯洛文尼亚共和国',
    'Solomon Islands' => '所罗门群岛',
    'Somalia' => '索马里联邦共和国',
    'South Africa' => '南非',
    'Spain' => '西班牙',
    'Sri Lanka' => '斯里兰卡',
    'St. Helena' => '圣赫勒拿岛',
    'Sudan' => '苏丹',
    'Suriname' => '苏里南共和国',
    'Svalbard and Jan Mayen Islands' => '斯瓦尔巴群岛',
    'Swaziland' => '斯威士兰王国',
    'Sweden' => '瑞典',
    'Switzerland' => '瑞士',
    'Syrian Arab Republic' => '阿拉伯叙利亚共和国',
    'Taiwan' => '中国台湾',
    'Tajikistan' => '塔吉克斯坦共和国',
    'Tanzania, United Republic of' => '坦桑尼亚联合共和国',
    'Thailand' => '泰国',
    'Togo' => '多哥共和国',
    'Tokelau' => '托克劳群岛',
    'Tonga' => '汤加王国',
    'Trinidad and Tobago' => '特立尼达和多巴哥',
    'Tunisia' => '突尼斯共和国',
    'Turkey' => '土耳其共和国',
    'Turkmenistan' => '土库曼斯坦',
    'Turks and Caicos Islands' => '特克斯和凯科斯群岛',
    'Tuvalu' => '图瓦卢',
    'Uganda' => '乌干达',
    'Ukraine' => '乌克兰',
    'United Arab Emirates' => '阿联酋',
    'United Kingdom' => '英国',
    'United States' => '美国',
    'United States Minor Outlying Islands' => '美国本土外小岛屿',
    'Uzbekistan' => '乌兹别克斯坦共和国',
    'Vanuatu' => '瓦努阿图共和国',
    'Vatican City State (Holy See)' => '梵蒂冈',
    'Venezuela' => '委内瑞拉',
    'Viet Nam' => '越南',
    'Virgin Islands (British)' => '英属维尔京群岛',
    'Virgin Islands (U.S.)' => '维尔京群岛（美国)',
    'Wallis and Futuna Islands' => '瓦利斯和富图纳群岛',
    'Western Sahara' => '西撒哈拉',
    'Yemen' => '也门共和国',
    'Democratic Republic of Congo' => '刚果民主共和国',
    'Zambia' => '赞比亚共和国',
    'Montenegro' => '黑山共和国',
    'Serbia' => '塞尔维亚共和国',
    'Aaland Islands' => '奥兰群岛',
    'Curacao' => '库拉索',
    'South Sudan' => '南苏丹共和国',
    'Isle of Man' => '马恩岛',
    'Ascension Island (British)' => '阿森松岛（英国）',
    'Kosovo, Republic of' => '科索沃共和国',
    'Canary Islands' => '加那利群岛',

    //公共页脚
    'GuestBook' => '留言版',
    'About' => '关于我们',
    'Store' => '在线商店',
    'Community' => '关注我们',
    //导航部分的翻译
    'All CATEGORIES' => '菜单',
    'close' => '关闭',
    'back' => '返回',
    'product center' => '产品中心',
    'support' => '服务与支持',
    'article'=>'文章/博客',
    'blog' => '博客',
    'News'=>'新闻与评测',
    'More'=>'更多',
    'Learn More'=>'了解更多',
    'language' => '语言选择',
    'go' => '搜索',
    'search tips' => '请输入产品型号或者名称进行搜索',
    'driver download' => '驱动下载',
    'manual' => '说明书与手册',
    'firmware' => '固件下载',
    'list of firmware' => '固件列表',
    'Firmware Details and Download' => '固件下载详情',
    'list of article' => '新闻列表',
    'list or video'=>'视频列表',
    'Update Time'=>'更新时间',
    'video'=>'视频',
    'Details' => '详细信息',
    'Notices' => '注意',
    'faq' => '常见问题',
    'filter by' => '分类筛选',
    'All Category' => '所有',
    'sort by' => '排序',
    'result' => '查询结果',
    'Date, old to new' => '按更新日期顺序排列',
    'Date, new to old' => '按更新日期倒序排列',
    'File Size' => '文件大小 ',
    'Version' => '版本',
    'Post Date' => '更新日期',
    'System Requirements' => '系统要求',
    'Available' => '支持的型号',
    'Download' => '下载连接',
    'ALL' => '所有',
    'WINDOWS' => 'Window系统',
    'MAC' => 'MAC',
    'Description' => '描述',
];
